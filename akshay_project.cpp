...
// creating a module for adding a flavour into charts
void add_flavor (char *name, int stock)
{
  ...
  for (int i = 0; i < num_flavors; i++)
	...
	{
	  printf ("This Flavour is already available in our stock.\n");
	  return;
	}
    }
  Flavor new_flavor;
  ...
  num_flavors++;
  printf ("\n*** %s Flavor ICE-CREAM of Stock [%d] successfully added to the cart.\n",new_flavor.name,new_flavor.stock);
}


// creating a module for removing a flavour from charts
void remove_flavor (char *name)
{
  for (int i = 0; i < num_flavors; i++)
      ...
      if (strcmp (flavors[i].name, name) == 0)
	{
	  ...
	  num_flavors--;
	  printf (" * %s Flavor removed successfully *.\n",name);
	  return;
	}
    }
...
// Creating a module for updating already existing flavours
void update_flavors (char *name, char *updated_name, int updated_stock)
{
  for (int i = 0; i < num_flavors; i++)
    {
      if (strcmp (flavors[i].name, name) == 0)
...
// Creating a Format(main menu) specifier
void format(){
    printf("\n*************************************************************");
    printf("\n\t PRAMOD'S ICE-CREAM MANAGEMENT COMPANY");
    printf("\n*************************************************************\n");
};
...
int main ()
{
  ...
  do
    {
      ...
      printf ("6. Exit\n");
      printf ("\n\t%c%c%c%C%cEnter your Choice : ",62,62,62,62,62);
      scanf ("%d", &choice);
      switch (choice)
	{
	case 1:
  ...
  case 4:
	...
	default:
	  printf ("\n* You have opted for an Invalid choice!\n**Please choose any valid choice mentioned above\n\n");
	  break;
	}
    }
  while (choice != 6);
